...
test("custom route is a canvas workspace, not a card gallery", () => {
  ...
  assert.match(builder, /Dashboard canvas/);
  assert.match(builder, /grid-cols-12/);
  assert.match(builder, /Canvas \/ 12 columns/);
  assert.match(builder, /draggable={editMode}/);
  assert.match(builder, /onDrop=/);
  assert.match(builder, /Build tools/);
  assert.match(builder, /Add workspace surface/);
  assert.match(builder, /Fixture workspace/);
