...
type WidgetPaletteEntry = { type: WidgetType; label: string; hint: string };

function WidgetBody({ widget, labels }: Readonly<{ widget: Widget; labels: Messages["screens"]["custom"] }>) {
  switch (widget.type) {
    case "entity-list":
    ...
    case "map-mini":
      return <div className="grid h-full min-h-20 place-items-center text-[11px]" style={{ color: "var(--text-secondary)" }}>{labels.widgetBody.mapMini}</div>;
    case "sparkline-grid":
    ...
    case "huginn-mini":
      return <div className="min-h-20 text-[11px] leading-relaxed" style={{ color: "var(--text-secondary)" }}>{labels.widgetBody.huginnMini}</div>;
    default:
      return null;
  }
}
...
export function DashboardBuilder({ labels }: Readonly<{ labels: Messages["screens"]["custom"] }>) {
  ...
  const widgetPalette: WidgetPaletteEntry[] = [
    { type: "entity-list", ...labels.widgets.entityList },
    { type: "alert-queue", ...labels.widgets.alertQueue },
    { type: "map-mini", ...labels.widgets.mapMini },
    { type: "sparkline-grid", ...labels.widgets.sparklineGrid },
    { type: "daily-diff", ...labels.widgets.dailyDiff },
    { type: "sector-rotation", ...labels.widgets.sectorRotation },
    { type: "source-health", ...labels.widgets.sourceHealth },
    { type: "huginn-mini", ...labels.widgets.huginnMini }
  ];

  if (!active) {
    return <div className="border-y px-5 py-8 text-sm" style={{ borderColor: "var(--line-soft)", color: "var(--text-secondary)" }}>{labels.noDashboard}</div>;
  }
  ...
  return (
    <div className="min-w-0">
      <header className="flex flex-wrap items-start justify-between gap-4 border-b px-4 py-4 sm:px-6" style={{ borderColor: "var(--line-soft)" }}>
        <div>
          <div className="mono text-[11px] uppercase tracking-[0.14em]" style={{ color: "var(--text-tertiary)" }}>{labels.eyebrow}</div>
          <h1 className="mt-1 text-[21px] font-medium" style={{ color: "var(--text-primary)" }}>{active.name}</h1>
          <p className="mt-1 text-[12px]" style={{ color: "var(--text-secondary)" }}>{labels.description}</p>
        </div>
        <div className="mono text-[11px] uppercase tracking-[0.1em]" style={{ color: "var(--text-tertiary)" }}>{labels.fixtureWorkspace}</div>
      </header>

      <div className="border-b px-4 py-3 sm:px-6" style={{ borderColor: "var(--line-soft)" }}>
        <div className="flex flex-wrap items-center gap-2" role="tablist" aria-label={labels.savedDashboards}>
          ...
          <button type="button" onClick={toggleEdit} aria-pressed={editMode} className="min-h-11 border px-3 text-[12px] transition-colors duration-[120ms] hover:bg-[var(--surface-hover)] focus-visible:outline-2 focus-visible:outline-[var(--signal)] motion-reduce:transition-none" style={{ background: editMode ? "var(--signal-wash)" : "transparent", borderColor: editMode ? "var(--signal)" : "var(--line-soft)", color: editMode ? "var(--signal)" : "var(--text-secondary)" }}>
            {editMode ? labels.finishLayout : labels.editLayout}
          </button>
        ...
        <p aria-live="polite" className="mono mt-2 text-[11px] uppercase tracking-[0.1em]" style={{ color: editMode ? "var(--signal)" : "var(--text-tertiary)" }}>
          {editMode ? labels.editingStatus : String(active.widgets.length) + " " + labels.readOnlySuffix}
        </p>
      </div>

      {editMode ? (
        <aside aria-label={labels.addSurface} className="border-b px-4 py-3 sm:px-6" style={{ borderColor: "var(--line-soft)" }}>
          <div className="mono mb-2 text-[11px] uppercase tracking-[0.14em]" style={{ color: "var(--text-tertiary)" }}>{labels.buildTools}</div>
      ...
      <div className="overflow-x-auto px-4 py-4 sm:px-6 sm:py-5" aria-label={labels.canvas}>
        <div className="min-w-[640px] border" style={{ borderColor: "var(--line-soft)", background: "var(--field)" }}>
          <div className="flex items-center justify-between border-b px-3 py-2" style={{ borderColor: "var(--line-soft)" }}>
            <span className="mono text-[11px] uppercase tracking-[0.12em]" style={{ color: "var(--text-tertiary)" }}>{labels.canvasColumns}</span>
            <span className="mono text-[11px] uppercase tracking-[0.1em]" style={{ color: "var(--text-quaternary)" }}>{labels.sourceObjectSignal}</span>
          </div>
          {active.widgets.length ? (
            <div className="grid min-h-[420px] grid-cols-12 auto-rows-[80px] gap-px p-px" style={{ background: "var(--line-soft)" }}>
              {active.widgets.map((widget) => (
                <article
                  key={widget.id}
                  draggable={editMode}
                  aria-label={widget.type + " " + labels.widgetSurface}
                ...
                >
                  <div className="mb-2 flex min-h-11 items-start justify-between gap-2 border-b pb-2" style={{ borderColor: "var(--line-faint)" }}>
                    ...
                    {editMode ? <button type="button" onClick={() => removeWidget(widget.id)} className="min-h-11 px-1 text-[12px] text-[var(--critical)] underline-offset-2 hover:underline focus-visible:outline-2 focus-visible:outline-[var(--signal)]">{labels.remove}</button> : null}
                  </div>
                  <WidgetBody widget={widget} labels={labels} />
                </article>
              ))}
            </div>
          ) : (
            <div className="grid min-h-[280px] place-items-center px-5 py-12 text-center">
              <div>
                <p className="text-sm" style={{ color: "var(--text-primary)" }}>{labels.canvasEmpty}</p>
                <p className="mt-1 text-[12px]" style={{ color: "var(--text-secondary)" }}>{labels.canvasEmptyHint}</p>
              </div>
            </div>
          )}
        </div>
