          ...
          {HUGINN_PRESETS.map((preset) => {
            ...
            return (
              <div key={preset.id} className="flex min-h-14 items-center justify-between gap-3 border-b px-3 py-2 last:border-b-0" style={{ borderColor: "var(--line-faint)" }}>
                <div className="min-w-0">
                  <div className="truncate text-[12px]" style={{ color: isDisabled ? "var(--text-tertiary)" : "var(--text-primary)" }}>{preset.label}</div>
                  <div className="mono mt-1 truncate text-[11px]" style={{ color: "var(--text-quaternary)" }}>{preset.template.slice(0, 70)}…</div>
                </div>
          ...
          {customs.map((template) => editing === template.id ? (
            ...
            <div key={template.id} className="flex min-h-14 items-center justify-between gap-3 border-b px-3 py-2 last:border-b-0" style={{ borderColor: "var(--line-faint)" }}>
              ...
              <div className="flex shrink-0 gap-1">
                <button type="button" onClick={() => startEdit(template)} className="min-h-11 px-2 text-[12px] text-[var(--text-secondary)] underline-offset-2 hover:underline focus-visible:outline-2 focus-visible:outline-[var(--signal)]">Edit</button>
  ...
  return (
    <div className="grid gap-3 border-l-2 px-3 py-4" style={{ background: "var(--surface-inset)", borderColor: "var(--signal)" }}>
      <label className="grid gap-1">
        ...
        <input type="text" value={draft.label} onChange={(event) => setDraft({ ...draft, label: event.target.value })} className="min-h-11 border px-3 text-[12px] outline-none focus-visible:border-[var(--signal)]" style={fieldStyle} placeholder="e.g. Weekly Sector Review" />
      </label>
      <label className="grid gap-1">
        <span className="mono text-[11px] uppercase tracking-[0.1em]" style={{ color: "var(--text-tertiary)" }}>{messages.template}</span>
        <textarea value={draft.template} onChange={(event) => setDraft({ ...draft, template: event.target.value })} rows={3} className="min-h-24 w-full resize-y border px-3 py-2 text-[12px] outline-none focus-visible:border-[var(--signal)]" style={fieldStyle} placeholder="e.g. Analyze {sector} capital fixation trends this week..." />
      </label>
      <label className="grid gap-1">
