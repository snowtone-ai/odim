...
- Entity markers as `symbol` layer with per-substrate SDF icons (SVG ↁESDF at build time)
...
### Decision: **Skeleton A  EMapLibre Native Layers**
- **Why**: Odim's demo entity count is 10 E0. Native symbol+line layers give correct geo-positioning (zero drift), GPU rendering, built-in clustering, and the smallest bundle. The "military software" aesthetic demands clean, crisp, performant rendering  Enot DOM manipulation. deck.gl's 500KB overhead and main-thread triangulation are unjustified for this scale. Research confirms MapLibre is 2x faster than deck.gl for tile rendering.
...
## Phase 1: Map  EPalantir Gotham Quality (Priority: P0)
...
| T035 | ready | Sonnet | T034 | components/ui/reality-map.tsx | Add MapLibre clustering: GeoJSON source with `cluster: true, clusterMaxZoom: 8, clusterRadius: 50`. Cluster circle layer with `circle-radius` stepped by point_count. Cluster count label as symbol layer. Click cluster ↁEzoom to expand. Uncluster transition animation | pnpm typecheck; pnpm build; visual check | Clusters appear at low zoom; click to expand; individual entities at high zoom |
| T036 | ready | Sonnet | T034 | components/ui/reality-map.tsx, lib/map/connections.ts | Add connection line layer: GeoJSON LineString source from ontology links. Line color = source entity substrate color. Line width = confidence ÁE3. Line opacity = 0.4 E.8 based on confidence. Dash array for low-confidence links. Great-circle interpolation for >1000km connections | pnpm typecheck; pnpm build; visual check | Connection lines visible between related entities; width/opacity reflect confidence |
| T037 | ready | Sonnet | T036 | components/ui/reality-map.tsx | Entity interaction: (a) hover ↁEhighlight entity + connected lines + tooltip with name/score/confidence/substrate; (b) click ↁEselect entity, dim unrelated entities, highlight connected entities and lines, show detail in sidebar; (c) `mouseenter`/`mouseleave` cursor change. Use `queryRenderedFeatures` for hit testing | pnpm typecheck; pnpm build; visual check | Hover shows tooltip; click selects and highlights network; deselect on map click |
...
## Phase 2: Navigation Consolidation  E8 Screens ↁE5 (Priority: P0)
...
| T053 | ready | Sonnet | T052 | app/(dashboard)/*/page.tsx, components/ui/screen.tsx, lib/i18n/messages.ts | Text-level corrections: (a) Screen component: remove "Live · Source-backed" badge  Eredundant after Gotham-quality map; (b) Entity page: "Committed" metric ↁEshow actual amount not fallback "Source-backed" text; (c) Alerts: "Signal Chain" steps  Emake dynamic from actual alert evidence, not static i18n array; (d) Settings: remove verbose copy paragraphs under each panel  Epanel content speaks for itself | pnpm typecheck; pnpm build; visual check | No redundant badges; metric values are real; no placeholder copy |
...
Phase 0: T031 (commit existing fixes)
  ↁEPhase 1: T032 ↁET033 ↁET034 ↁET035 ↁET036 ↁET037 ↁET038 ↁET039 ↁET040
  ↁE(T041 can start after T031, parallel with late Phase 1)
Phase 2: T041 ↁET042 ↁET043 ↁET044 ↁET045 ↁET046
  ↁEPhase 3: T047 ↁET048 ↁET049 (depends on T041 for nav)
Phase 4: T050 ∥ T051 ∥ T052 ↁET053 (T050/T051/T052 are parallelizable)
  ↁEPhase 5: T054 ↁET055
```
...
## Release-Critical Refactoring & Review (2026-05-27)

| ID | Status | Owner | Scope | Evidence |
|---|---|---|---|---|
| RC-001-RC-031 | verified | Codex | Security, CI, env validation, auth/API controls, Huginn prompt safety, AI/scraper timeouts, transactional ingestion, frontend resilience, RLS/parser/bias coverage | Implemented as focused commits; all release readiness gates passed |
...
- [x] All RC-001 through RC-031 tasks completed.
- [x] P0 release blockers closed.
- [x] P1 hardening items closed or covered by verification.
- [x] AGENTS.md records the release-critical coding rules for future work.
- [x] docs/state.md and docs/repo-map.md updated.
- [x] prompt.md removed after completion.
