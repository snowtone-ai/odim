...
Odimは **1つのWebアプリケーション** で、5つの画面（Screen）からなる（v3.0）。

```
ØDIM Web App
│
├── /map      … Reality Map — 世界の資本移動をMapLibreで可視化（Palantir Gotham品質）
├── /entity   … Entity Intelligence — 企業・SPVの詳細 + Capital Flow + Watchlist統合
├── /alerts   … Signal Alerts — 優先度付きアラート
├── /huginn   … Huginn — 自然言語でOntologyに質問するAIエージェント
└── /settings … Settings — アラートルール/APIキー/権限/Audit Trail統合
```

**v3.0での統合（2026-05）**:
- Capital Flow → Entity Intelligence に統合（セクターヒート + Reality gap）
- Watchlist & Briefs → Entity Intelligence に統合（Watchedフィルタ + Daily Brief）
- Audit Trail → Settings に展開（省略なし全ログ表示）
- 独立画面として削除: capital-flow, watchlist, audit
...
### 3.1 中核機能（Core）— v3.0（5画面）

| 機能 | 画面 | 説明 |
|---|---|---|
| **Reality Map** | /map | MapLibre GLネイティブレイヤー（Palantir Gotham品質）。シンボルレイヤー・クラスタリング・接続ライン・ホバーポップアップ・クリック選択・マップ検索バー。CARTOダークマターベースマップ。 |
| **Entity Intelligence** | /entity | 企業/SPVの詳細。Reality Score、Ontology Links。セクターヒートマップ（Capital Flow統合）、Narrative-Reality Gapテーブル、Watchedフィルタ + Daily Brief（Watchlist統合）。 |
| **Signal Alerts** | /alerts | 優先度付き（Critical/High/Medium/Low）アラートキュー。証拠チェーン・親会社候補スコア付き。 |
| **Huginn** | /huginn | 自然言語でOntologyに質問するインタラクティブコンソール。クエリ入力・トレース・出典・信頼度バー。 |
| **Settings** | /settings | アラートルール/APIキー/権限/Seed Memoryマネージャ/Audit Trail全ログ。 |
| **CommandPalette** | 全画面 | Cmd+Kでグローバル検索（エンティティ・アラート・設定）。 |
| **EntityLink** | 全画面 | /entity?id=X への画面間ナビゲーションコンポーネント。 |
...
組織ごとに育つAI。詳細は `source-05-huginn-munin.md`。最初から実装する。
